...
**Local provider** (`AIProvider.local`, Settings ▸ AI ▸ Local → `LocalModelsSettingsView`): a catalog of coding GGUF models (`LocalModelCatalog`, pinned URLs + SHA-256) downloaded by `FileDownload` (resumable `.part`, hashed while streaming) into `~/Library/Application Support/MyGit/LocalAI/models`; the first download also fetches llama.cpp's prebuilt `llama-server` (pinned release) into `LocalAI/runtime`. `LocalLLMServer` (actor) runs it as a child process on a free loopback port, one model at a time, unloads after 15 min idle, and is killed in `applicationWillTerminate`. `AICommitMessageRepository.complete` resolves `.local` to that server's OpenAI-compatible URL, so every AI feature works offline. No key: `SettingsViewModel.config(for:)` skips the keychain for providers where `needsKey` is false.
...
Source navigation: "Run with Inspector" (`RunViewModel.runWithInspector` → `ProjectToolchain.inspectRunScript`) builds a mirror in `<repo>/.mygit/inspect/src` whose SwiftUI views carry `_PreferenceWritingModifier<__MyGitSourceKey>` tags (`path:line:col`), made by `ios-inspector/Tools/SourceTagger`; `run.sh` bundles the tagger + `mygit-inspect-run.sh` into Resources. `UIInspectorDetails.swift` reads tags (`sourceStack`, nearest first) and the Figma-style detail sections; without tags the context menu falls back to text/type search via `InspectorSourceNavigating` (`App/InspectorSourceNavigator.swift`).

Token roots with several branches ("◆ one of") are narrowed in three steps, strongest first: (1) `runtimeBranch(among:value:)` — the tagger's branch probes (`__mB`, see `ios-inspector/README.md`) report which `return` produced which value (`InspectorSnapshot.branches`, trusted only when every root is `probed`); (2) `InspectorBranchPruner` rules out roots that can't produce the on-screen value (`nil`, other literals, string templates that don't fit); (3) the ✨ Pick button (`UIInspectorAIPick.swift`) asks the active AI provider with the code around each root + on-screen texts, labeled as a suggestion (`InspectorTokenResolution.pickedBy`).
